    ...
    >
      ...
      >
        <el-form-item label="疫情信息标题" prop="gtitle">
          <el-col :span="16">
            <el-input v-model="operateForm.gtitle" placeholder="请输入疫情信息标题"></el-input>
          </el-col>
        </el-form-item>

        <el-form-item label="疫情信息内容" prop="gcontent">
          <el-col :span="16">
            <el-input placeholder="请输入疫情信息内容" type="textarea" v-model="operateForm.gcontent"></el-input>
          </el-col>
        </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        gtitle: [
          { required: true, message: "疫情信息标题不能为空", trigger: "blur" },
        ],
        gcontent: [
          { required: true, message: "疫情信息内容不能为空", trigger: "blur" },
        ],
      },
  ...
  methods: {
    ...
    onSubmit() {
      this.$refs.form.validate((valid) => {
        ...
        if (this.operateType === "edit") {
          gonggaoupdate(this.operateForm).then((res) => {
            if (res.code === 0) {
              this.$message.success(res.msg);
              this.loadData();
            } else {
          ...
          gonggaoadd(this.operateForm).then((res) => {
            if (res.code === 0) {
              this.$message.success(res.msg);
              this.loadData();
            } else {
    ...
    loadDataByid(row) {
      gonggaoByid({ id: row.id }).then((res) => {
        if (res.code != 0) {
